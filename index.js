...
async function revertWarning(guildId, userId) {
  ...
  if (
    !warnings[userId] ||
    !Array.isArray(warnings[userId].history) ||
    warnings[userId].history.length === 0
  ) {
    return false;
  }
...
async function saveOverrideCodes(overrideData) {
  await fs.writeFile("./override-codes.json", JSON.stringify(overrideData, null, 2));
}
      ...
      if (codeEntry.autoGenerated && !codeEntry.sentToChannel && !codeEntry.used) {
        ...
        if (timeSinceGeneration >= oneDayInMs) {
          try {
            ...
            console.error(`Failed to send auto-generated override code to channel:`, error);
          }
        }
      }
    }
  ...
  */
  ...
  switch (interaction.commandName) {
    ...
    case "warn": {
      ...
      // Determine actor override / real-staff membership
      const actorId = interaction.user.id;
      const actorIsOverridden = isUserOverridden(actorId);

      // Check if actor has any *real* staff role (roles defined in roleHierarchy)
      const actorHasRealStaffRole = interaction.member
        ? interaction.member.roles.cache.some((r) => staffRoleIds.includes(r.id))
        : false;

      // Silent mode: only force silent for overridden users who are NOT real staff
      let silent = interaction.options.getBoolean("silent") || false;
      if (actorIsOverridden && !actorHasRealStaffRole) {
        silent = true;
      }
      ...
      // Decide whether to save a warning and create a case:
      // - If actor is NOT overridden => normal behavior (save & create).
      // - If actor IS overridden AND has a real staff role => allow save & create.
      // - If actor IS overridden AND has NO real staff role => skip save/create (Option B).
      if (!actorIsOverridden || (actorIsOverridden && actorHasRealStaffRole)) {
        // Add warning + create case (normal saved behavior)
        ...
        // Overridden non-staff: do not persist a warning or create a case (Option B)
      ...
      try {
        ...
        // sendLogIfNotOverridden will skip LOG_CHANNEL if actor is overridden and not staff
      ...
      if (timeoutApplied) {
        responseEmbed.addFields({ name: "⏱️ Timeout Applied", value: `${timeoutMinutes} minute(s)`, inline: true });
      }

      if (!silent && !dmSent) {
        responseEmbed.addFields({ name: "⚠️ DM Status", value: "Could not send DM (user may have DMs disabled)", inline: false });
      } else if (silent) {
        responseEmbed.addFields({ name: "🔇 Silent Mode", value: "No DM sent to user", inline: false });
      } else {
        responseEmbed.addFields({ name: "✅ DM Status", value: "DM sent successfully", inline: false });
      }
      ...
      try {
        ...
        const purgeEmbed = new EmbedBuilder()
          ...
          .addFields(
            ...
            { name: "Actually Deleted", value: `${deleted.size}`, inline: true },
            { name: "Moderator", value: interaction.user.tag, inline: true }
          )
          .setTimestamp()
        ...
        if (targetUser) {
          purgeEmbed.addFields({ name: "Target User", value: targetUser.tag, inline: true });
        }
        ...
        if (targetUser) {
          errorEmbed.addFields({ name: "Target User", value: targetUser.tag, inline: true });
        }
    ...
    case "deletecase": {
      ...
      if (!deletedCase) {
        const notFoundEmbed = new EmbedBuilder()
          ...
          .setDescription(`Case #${caseNumber} does not exist or has already been deleted.`)
          .setTimestamp();
      ...
      const deleteEmbed = new EmbedBuilder()
        ...
        .setDescription(`**Case #${caseNumber}** has been permanently deleted and no longer exists in the system.`)
        .addFields(
          { name: "Type", value: `${typeEmoji} ${deletedCase.type.toUpperCase()}`, inline: true },
      ...
      if (deletedCase.severity) {
        deleteEmbed.addFields({ name: "Severity", value: sevText, inline: true });
      }

      if (deletedCase.duration) {
        deleteEmbed.addFields({ name: "Duration", value: durText, inline: true });
      }
      ...
      if (revertWarn && deletedCase.type === "warn") {
        ...
        if (reverted) {
          deleteEmbed.addFields({ name: "✅ Warning Reverted", value: "Warning count decreased by 1", inline: false });
        } else {
          deleteEmbed.addFields({ name: "⚠️ Warning Not Reverted", value: "User had no warnings to revert", inline: false });
        }
    ...
    case "generatebancode": {
      ...
      try {
        ...
        // actorIsStaff should reflect real staff role membership (not overridden synthetic)
        const actorHasRealStaffRole = interaction.member
          ? interaction.member.roles.cache.some((r) => staffRoleIds.includes(r.id))
          : false;

        // allow posting when: not overridden (regular staff) OR overridden but also real staff role
        const shouldPostPublicly = !isOverridden || (isOverridden && actorHasRealStaffRole);
    ...
    case "kick": {
      ...
      try {
        ...
        if (targetStaffRole && !isUserOverridden(interaction.user.id)) {
          const errorEmbed = new EmbedBuilder()
            ...
            .setDescription(`**${targetUser.tag}** is a staff member and cannot be kicked.`)
            .addFields(
              { name: "Target Role", value: targetStaffRole.name, inline: true },
              { name: "Reason", value: "Staff members are immune to kicks", inline: false }
            )
            .setTimestamp();
      ...
      try {
        ...
        const errorEmbed = new EmbedBuilder()
          ...
          .addFields(
            { name: "Reason", value: "Missing permissions, user not in server, or user has higher role", inline: false },
            { name: "User", value: targetUser.tag, inline: true }
          )
          .setTimestamp();
    ...
    case "ban": {
        ...
        if (overrideCode) {
          ...
          if (codeData && codeData.command === "ban") {
            ...
            try {
              const codeChannel = await interaction.guild.channels.fetch(OVERRIDE_CODE_CHANNEL);
              if (codeChannel) {
                // Option B: do NOT send usage notification if actor is overridden and not a real staff member
                const actorOverridden = isUserOverridden(interaction.user.id);
                const actorHasRealRole = interaction.member ? interaction.member.roles.cache.some((r) => staffRoleIds.includes(r.id)) : false;
                ...
                if (!actorOverridden || (actorOverridden && actorHasRealRole)) {
                  const usageEmbed = new EmbedBuilder()
                    ...
                    .setDescription(`An override code has been consumed for a ban action.`)
                    .addFields(
                      { name: "Code Used", value: `\`${overrideCode}\``, inline: false },
                      { name: "Used By", value: `${interaction.user.tag} (${interaction.user.id})`, inline: true },
                      { name: "Originally Generated By", value: codeData.generatedBy, inline: true }
                    )
                    .setTimestamp()
                    .setFooter({ text: "A new code will be generated and sent after 24 hours" });
              ...
              console.error("Failed to send override code usage notification:", error);
            }
      ...
      if (isHackban) {
        ...
        try {
          ...
          const errorEmbed = new EmbedBuilder()
            ...
            .addFields(
              { name: "Reason", value: "Invalid user ID, missing permissions, or user already banned", inline: false },
        ...
        try {
          ...
          if (targetStaffRole && !isUserOverridden(interaction.user.id)) {
            const errorEmbed = new EmbedBuilder()
              ...
              .setDescription(`**${targetUser ? targetUser.tag : targetUserId}** is a staff member and cannot be banned.`)
              .addFields(
                { name: "Target Role", value: targetStaffRole.name, inline: true },
                { name: "Reason", value: "Staff members are immune to bans", inline: false }
              )
              .setTimestamp();
        ...
        try {
          ...
          const errorEmbed = new EmbedBuilder()
            .setColor(0xe74c3c)
            .setTitle("❌ Ban Failed")
            .setDescription(`Failed to ban **${targetUser ? targetUser.tag : targetUserId}**`)
            .addFields(
              { name: "Reason", value: "Missing permissions, user already banned, or user has higher role", inline: false },
              { name: "User", value: targetUser ? targetUser.tag : targetUserId, inline: true }
            )
            .setTimestamp();
      ...
      try {
        const logEmbed = new EmbedBuilder()
          ...
          .addFields(
            {
              name: "Member",
              value: isHackban ? bannedUsername : `${targetUser ? targetUser.tag : targetUserId} (${bannedUserId})`,
              inline: true
            },
            { name: "Case #", value: caseNumber ? `#${caseNumber}` : "N/A", inline: true },
            { name: "Moderator", value: `${interaction.user.tag}`, inline: true },
        ...
        if (deleteDays > 0) {
          logEmbed.addFields({ name: "Message Deletion", value: `${deleteDays} day(s) of messages deleted`, inline: true });
        }
      ...
      const banEmbed = new EmbedBuilder()
        .setColor(0xe74c3c)
        .setTitle(isHackban ? "🔨 Hackban Issued" : "🔨 Member Banned")
        .setDescription(isHackban ? `Successfully hackbanned user **${bannedUsername}**` : `Successfully banned **${targetUser ? targetUser.tag : targetUserId}** from the server`)
        .setThumbnail(targetUser ? targetUser.displayAvatarURL() : null)
        .addFields(
          { name: "Case Number", value: caseNumber ? `#${caseNumber}` : "N/A", inline: true },
          { name: "Moderator", value: interaction.user.tag, inline: true },
      ...
      if (deleteDays > 0) {
        banEmbed.addFields({ name: "Message Deletion", value: `${deleteDays} day(s)`, inline: true });
      }

      if (usedOverride) {
        banEmbed.addFields({ name: "🔑 Override Code", value: "Used override code (new code generated)", inline: true });
      }
              ...
              if (codeChannel) {
                ...
                if (!actorOverridden || (actorOverridden && actorHasReal)) {
                  const usageEmbed = new EmbedBuilder()
                    .setColor(0x2ecc71)
                    .setTitle("🔑 Override Code Used")
                    .setDescription(`An override code has been consumed for an unban action.`)
                    .addFields(
                      { name: "Code Used", value: `\`${overrideCode}\``, inline: false },
                      { name: "Used By", value: `${interaction.user.tag} (${interaction.user.id})`, inline: true },
                      { name: "Originally Generated By", value: codeData.generatedBy, inline: true }
      ...
      if (!userId) {
        try {
          ...
          const listEmbed = new EmbedBuilder()
            ...
            .setTimestamp()
            .setFooter({ text: `Use /unban user_id:<ID> reason:<reason> to unban a user` });
          ...
          const errorEmbed = new EmbedBuilder()
            .setColor(0xe74c3c)
            .setTitle("❌ Error")
            .setDescription("Failed to fetch the ban list. Make sure the bot has the required permissions.")
            .setTimestamp();
      ...
      try {
        ...
        try {
          const logEmbed = new EmbedBuilder()
            ...
            .addFields(
              { name: "Member", value: unbannedUser ? `${unbannedUser.tag} (${userId})` : `User ID: ${userId}`, inline: true },
              { name: "Moderator", value: `${interaction.user.tag}`, inline: true },
              { name: "Reason", value: reason }
            )
            .setTimestamp()
        ...
        const unbanEmbed = new EmbedBuilder()
          .setColor(0x2ecc71)
          .setTitle("✅ Member Unbanned")
          .setDescription(`Successfully unbanned **${unbannedUser ? unbannedUser.tag : userId}**`)
          .setThumbnail(unbannedUser ? unbannedUser.displayAvatarURL() : null)
          .addFields({ name: "Moderator", value: interaction.user.tag, inline: true }, { name: "Reason", value: reason })
          .setTimestamp()
          .setFooter({ text: `User ID: ${userId}` });

        if (usedOverride) {
          unbanEmbed.addFields({ name: "🔑 Override Code", value: "Used override code (new code generated)", inline: true });
        }
        ...
        const errorEmbed = new EmbedBuilder()
          ...
          .addFields(
            { name: "Reason", value: "User is not banned, invalid user ID, or missing permissions", inline: false },
